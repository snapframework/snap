...
-- interpreter for the load function, and returns the initializer
-- along with the interpreter's proxy handler and cleanup actions.
-- The behavior of the proxy actions will change to reflect changes in
-- the watched files, including running each newly-interpreted load
-- function on the value generated by the initializer.
--
